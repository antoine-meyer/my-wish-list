...
class ViewParticipant{
    ...
    public function render(array $vars){
        //LISTES
        if(get_class($this->model[0][0])==="slim\models\Liste" || get_class($this->model[0])==="slim\models\Liste"){
    ...
    private function htmlUnItem(\slim\models\Item $item): string{
        $html = <<<END
    ...
    private function htmlUneListe(\slim\models\Liste $liste): string{
        $html = <<<END
            <section class="content">
    ...
    private function htmlUneListeEtItems(\slim\models\Liste $liste): string{
        $html = <<<END
            <section class="content">
                <h3>Liste {$liste->no} : {$liste->titre}</h3>
                <p>Description : {$liste->description}</p>
